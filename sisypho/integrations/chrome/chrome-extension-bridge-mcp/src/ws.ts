...
class WSClient {
  ...
  connect(port: number) {
    ...
    this._ws.on("connection", (ws) => {
      ...
      // Clean up pending callbacks when connection is lost
      this._socket.on("close", () => {
        this._cleanupPendingCallbacks("WebSocket connection closed");
      });
      
      this._socket.on("error", () => {
        this._cleanupPendingCallbacks("WebSocket connection error");
      });
    });
  ...
  private _cleanupPendingCallbacks(errorMessage: string) {
    // Reject all pending callbacks with the error message
    for (const [id, callback] of this._callbacks) {
      // Since we wrapped the callback, we need to create a reject function
      // The callback expects a response object, so we create one with an error
  ...
  private _call(
    ...
    return new Promise((resolve, reject) => {
      ...
      // Set timeout for the call (3 seconds)
      const timeout = setTimeout(() => {
        if (this._callbacks.has(id)) {
          this._callbacks.delete(id);
          reject(new Error("WebSocket call timeout - extension may be disconnected"));
        }
      }, 3000);
      
      // Wrap resolve to clear timeout
      const originalResolve = resolve;
      this._callbacks.set(id, (value: unknown) => {

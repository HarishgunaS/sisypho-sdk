...
class RecorderContext:
    def __init__(self):
        ...
        try:
            ...
            accessibility_server = "sisypho/integrations/macos/servers/.build/arm64-apple-macosx/release/AccessibilityMCPServer"
        
        # Build list of available servers
        self.default_servers = [accessibility_server]
        
        # Add Chrome extension server if available (optional)
        chrome_server = "sisypho/integrations/chrome/chrome-extension-bridge-mcp/node_modules/.bin/tsx sisypho/integrations/chrome/chrome-extension-bridge-mcp/examples/mcp.ts"
        chrome_server_dev = "integrations/chrome/chrome-extension-bridge-mcp/node_modules/.bin/tsx integrations/chrome/chrome-extension-bridge-mcp/examples/mcp.ts"
        
        # Check if Chrome extension is available
        from pathlib import Path
        if Path(chrome_server_dev.split()[0]).exists():
            self.default_servers.append(chrome_server_dev)
        elif Path(chrome_server.split()[0]).exists():
            self.default_servers.append(chrome_server)
        # If Chrome extension is not available, just skip it (it's optional)
        ...
        try:
            ...
            if not self.manager.initialize_all():
                # Show captured errors before raising exception
                init_errors = init_stderr_capture.getvalue()
                if init_errors:
                    print(f"\nServer initialization errors:\n{init_errors}", file=sys.__stderr__)

...
server.tool("getDOMTree", "Get the entire DOM tree of the current page", {}, async () => {
    const response = await client.callToolExtension("getDOMTree");
    return response;
});
server.tool("getPageInfo", "Get current page information including URL, title, and metadata", {}, async () => {
    const response = await client.callToolExtension("getPageInfo");
    return response;
});
// Element Selection
server.tool("getElementById", "Get an element by its ID", { id: z.string() }, async ({ id }) => {
    const response = await client.callToolExtension("getElementById", id);
    return ResponseSchema.parse(response);
});
server.tool("querySelector", "Get an element using a CSS selector", { selector: z.string() }, async ({ selector }) => {
    const response = await client.callToolExtension("querySelector", selector);
    return ResponseSchema.parse(response);
});
// Click Actions
server.tool("clickLink", "Click a link by its text content or href", { identifier: z.string() }, async ({ identifier }) => {
    const response = await client.callToolExtension("clickLink", identifier);
    return ResponseSchema.parse(response);
});
server.tool("clickButton", "Click a button by its text content or ID", { identifier: z.string() }, async ({ identifier }) => {
    const response = await client.callToolExtension("clickButton", identifier);
    return ResponseSchema.parse(response);
});
server.tool("clickElement", "Click any element using a CSS selector", { selector: z.string() }, async ({ selector }) => {
    const response = await client.callToolExtension("clickElement", selector);
    return ResponseSchema.parse(response);
});
// Form Interaction
server.tool("typeText", "Type text into a focused input field", {
    selector: z.string(),
    text: z.string(),
    options: z.object({
...
server.tool("submitForm", "Submit a form by its ID or containing element selector", { selector: z.string() }, async ({ selector }) => {
    const response = await client.callToolExtension("submitForm", selector);
    return ResponseSchema.parse(response);
});
// Navigation
server.tool("navigate", "Navigate to a specific URL", { url: z.string().url() }, async ({ url }) => {
    const response = await client.callToolExtension("navigate", url);
    return ResponseSchema.parse(response);
});
server.tool("goBack", "Navigate back in browser history", {}, async () => {
    const response = await client.callToolExtension("goBack");
    return ResponseSchema.parse(response);
});
server.tool("goForward", "Navigate forward in browser history", {}, async () => {
...
server.tool("evaluate", "Execute JavaScript code in the page context", {
    code: z.string(),
...
// Wait and Timing
server.tool("waitForElement", "Wait for an element to appear in the DOM", {
    selector: z.string(),
    options: z.object({
...
server.tool("retrieve_write_interaction_queue", "Retrieve and clear the queue of user interactions with the webpage", {}, async () => {
    const response = await client.callToolExtension("retrieve_write_interaction_queue");
    return response;
});
server.tool("getInteractionQueueSize", "Get the current size of the interaction queue", {}, async () => {
